...
class DuckDbProvider:
    ...
    def _connect(self, read_only=True):
        ...
        # 跨进程锁冲突重试：下载脚本等后台进程持有读写锁时，macOS 上只读连接也可能被拒绝。
        max_lock_retries = 5
        lock_retry_base_delay = 0.5
        for attempt in range(max_lock_retries + 1):
            try:
                ...
                lock_conflict = any(kw in err_text for kw in (
                    "could not set lock", "conflicting lock", "lock on file",
                ))

                # 同进程读写配置冲突：退回到读写连接复用。
                if bool(read_only) and "different configuration than existing connections" in err_text:
                    try:
                ...
                # 跨进程锁冲突：等待并重试。
                if lock_conflict and attempt < max_lock_retries:
                    wait_s = lock_retry_base_delay * (attempt + 1)
                    time.sleep(wait_s)
                    continue
        ...
        self.last_error = "DuckDB 连接失败: 锁冲突重试已耗尽"
        return None
    ...
    def fetch_kline_data(self, code, start_time, end_time, interval="1min"):
        ...
        if iv == "1min":
            ...
            df = self.fetch_daily_data(code, start_time, end_time)
        else:
            df = self._query_range(code, start_time, end_time, iv)

        if not df.empty:
            return df

        # 2. For minute intervals, try resampling from 1min
        if iv != "1min" and iv != "D":
            minute = self.fetch_minute_data(code, start_time, end_time)
            if not minute.empty:
                return Indicators.resample(minute, iv)

        # 3. Fallback: use daily data (even for minute strategies)
        daily = self.fetch_daily_data(code, start_time, end_time)
        if daily.empty:
            return pd.DataFrame()
        if iv == "D":
            return daily
        # For minute strategies, return daily bars so backtest can at least run
        return daily
